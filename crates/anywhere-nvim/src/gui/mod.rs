...
use anywhere_core::ui::input::{Mods, encode_key, encode_text};
use anywhere_core::ui::{UiState, redraw};
...
struct App {
    tx: Sender<Cmd>,
    /// 生かしておくだけ。drop するとアイコンが消え、ホットキーの登録が外れる。
    _tray: Tray,
    _hotkeys: Hotkeys,
...
impl ApplicationHandler<UserEvent> for App {
    ...
    fn window_event(
        ...
        match event {
            // 閉じるのは nvim の仕事（`AwQuit` = `ZQ`）。ここで exit しない。
