//! `anywhere_core::ui::redraw` を実 nvim の `redraw` 通知に対して検証する
//! （→ v2 計画 §4.1）。
//!
...
use anywhere_core::ui::UiState;
use anywhere_core::ui::redraw::apply;
use anywhere_core::{NvimConfig, NvimHandles, NvimServer};
use rmpv::Value;
...
async fn typed_ascii_shows_up_on_the_first_row() {
    let (mut server, mut handles) = start("anywhere-test-ui-redraw-ascii").await;
...
async fn typed_wide_chars_keep_their_continuation_cells() {
    let (mut server, mut handles) = start("anywhere-test-ui-redraw-wide").await;
    let mut state = attach_and_settle(&server, &mut handles).await;
