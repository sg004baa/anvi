...
impl NvimServer {
    ...
    pub async fn spawn(cfg: &NvimConfig) -> anyhow::Result<(Self, NvimHandles)> {
        ...
        if !init_lua.is_file() {
            bail!(
                "bundled init.lua not found at {} (runtime_dir must contain init.lua and lua/anywhere/)",
                init_lua.display()
            );
        }
    ...
    pub async fn start_session(
        ...
        self.nvim
            .exec_lua(
                "require('anywhere').start_session(...)",
                vec![lines, filetype],
            )
            .await
...
async fn register_host(nvim: &Neovim<HostWriter>) -> anyhow::Result<i64> {
    ...
    nvim.exec_lua("require('anywhere').set_host(...)", vec![Value::from(chan)])
        .await
        .context(
            "require('anywhere').set_host failed; the bundled init.lua did not establish the \
             contract. Check that runtime/init.lua and runtime/lua/anywhere/init.lua are the \
...
mod tests {
    ...
    async fn spawn_fails_fast_when_the_bundled_init_lua_is_missing() {
        let cfg = NvimConfig {
            ...
            appname: "anywhere-nvim-test".to_string(),
        };
    ...
    async fn spawn_reports_a_missing_nvim_executable() {
        let dir = std::env::temp_dir().join(format!("anywhere-core-test-{}", std::process::id()));
