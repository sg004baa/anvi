; anywhere-nvim のインストーラ（Inno Setup 6）。
;
;   ISCC.exe /DAppVersion=x.y.z /DStageDir=<配布物を並べたディレクトリ> installer\anywhere-nvim.iss
;
; StageDir は scripts/make-bundle.sh が組み立てたレイアウト（anywhere-nvim.exe /
...
[Setup]
AppId={{3A4422E5-B94B-49B4-B332-F5FC12C58577}
AppName=anywhere-nvim
AppVersion={#AppVersion}
AppPublisher=sg004baa
AppSupportURL=https://github.com/sg004baa/anywhere-nvim
ArchitecturesAllowed=x64compatible
ArchitecturesInstallIn64BitMode=x64compatible
DefaultDirName={localappdata}\Programs\anywhere-nvim
DefaultGroupName=anywhere-nvim
DisableProgramGroupPage=yes
PrivilegesRequired=lowest
UninstallDisplayIcon={app}\anywhere-nvim.exe
...
[Icons]
Name: "{userprograms}\anywhere-nvim"; Filename: "{app}\anywhere-nvim.exe"; WorkingDir: "{app}"

[Registry]
Root: HKCU; Subkey: "Software\Microsoft\Windows\CurrentVersion\Run"; ValueType: string; ValueName: "anywhere-nvim"; ValueData: """{app}\anywhere-nvim.exe"""; Flags: uninsdeletevalue; Tasks: startup

[Run]
Filename: "{app}\anywhere-nvim.exe"; Description: "anywhere-nvim を起動する"; WorkingDir: "{app}"; Flags: nowait postinstall skipifsilent
...
// 常駐中の anywhere-nvim を落とす。/T で子の nvim.exe も巻き込む。
...
begin
  Exec(
    ExpandConstant('{sys}\taskkill.exe'), '/F /T /IM anywhere-nvim.exe', '',
...
begin
  ...
  begin
    DeleteUserData :=
      MsgBox(
        'ローカル設定 (' + ExpandConstant('{localappdata}') + '\anywhere-nvim\init.lua など) と' + #13#10 +
        'shada / state (' + ExpandConstant('{localappdata}') + '\anywhere-nvim-data) も削除しますか? (既定: はい)' + #13#10 +
...
begin
  ...
  begin
    Log('anywhere-nvim: failed to delete user data directory: ' + Dir);
...
// %LOCALAPPDATA%\Programs\anywhere-nvim (インストール先) には一切触れない。
procedure CurUninstallStepChanged(CurUninstallStep: TUninstallStep);
begin
  if (CurUninstallStep = usPostUninstall) and DeleteUserData then
  begin
    DeleteDataDir(ExpandConstant('{localappdata}\anywhere-nvim'));
    DeleteDataDir(ExpandConstant('{localappdata}\anywhere-nvim-data'));
  end;
end;
